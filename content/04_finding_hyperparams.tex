...
in \hyperref[ap:additional_plots]{appendix \ref{ap:additional_plots}}.
\begin{figure}
    \centering
    % \begin{subfigure}[t]{0.32\textwidth}
...
I used noise quantiles to determine what percentage of all pixels would fall under a certain threshold.
The noise quantiles here mean all pixels that are not part of the true signal.
The resulting threshold would then serve as the core threshold \(Q_b\). The quantiles are calculated directly from the images by
