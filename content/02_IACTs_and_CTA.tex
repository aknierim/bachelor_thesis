...
\todo{confirm values and add source}\SI{100}{\giga\eV},
...
\end{figure}\todo{Adjust figure size of \autoref{fig:cherenkov_heitler}}
...
The \cta{} consists of three types of telescopes, shown in \autoref{fig:cta_telescopes}, each being
...
will feature the \glspl{mst} and \glspl{lst}. \todo{decide on final figure, check source for south}\autoref{fig:cta_layout} shows the
planned \todo[color=blue!40]{full or alpha}full layout of both sites of the \cta{} where \cta{} south is displayed with its \(\num{70}\)
...
consisting of \(\num{9}\) \glspl{mst} and \(\num{4}\) \glspl{lst}.
\todo[color=yellow]{decide on \autoref{fig:cta_layout}}
\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{build/array_layout.pdf}
    \caption{Alpha Configuration layout of the \cta{} north and full configuration of the \cta{}
    south site. \cta{} south would feature \(\num{70}\) \glspl{sst}, \(\num{25}\) \glspl{mst} and
    \(\num{4}\) \glspl{lst} \cite{cta_south_layout} its full configuration, while \cta{} north would
    feature \(\num{9}\) \glspl{mst} and \(\num{4}\) \glspl{lst} in it's Alpha Configuration \cite{cta_north_layout}.
    This figure was adapted from a figure by Kai Br\"ugge in his Ph.D. thesis \cite{bruegge_thesis}.}
    \label{fig:cta_layout}
\end{figure}

The \gls{mst}'s and \gls{lst}'s cameras are based on \todo[color=green]{write about camera tech}\gls{pmt} photodectors, while the \gls{sst}'s
camera is based on a \gls{sipm} photodetector.

\todo[color=purple!40]{write about scientific goals for the cta}
