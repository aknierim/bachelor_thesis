...
Also, I relied heavily on the python programming language, of which the most important libraries are listed below.
\begin{itemize}
    \item \texttt{numpy} ~\cite{numpy}
    \item \texttt{pandas} ~\cite{pandas}
    \item \texttt{matplotlib} ~\cite{matplotlib}
    \item \texttt{astropy} ~\cite{astropy1, astropy2}
    \item \texttt{pyirf} ~\cite{pyirf}
    \item \texttt{scikit-learn} ~\cite{scikit-learn}
\end{itemize}

For the processing of the datasets, I used the development version of \texttt{ctapipe}, specifically version
